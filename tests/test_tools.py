...
RELATIONSHIP_DETAIL = {"id": 1, "factTableId": 1, "dimensionGroupId": 1}
...
async def test_get_connection(service: SemanticService) -> None:
    respx.get(f"{BASE}/df-api/v1/projects/392/versions/948/connections/1").mock(
...
async def test_get_dimension_group(service: SemanticService) -> None:
    respx.get(f"{BASE}/df-api/v1/projects/392/versions/948/dimension-groups/1").mock(
...
async def test_get_fact_table(service: SemanticService) -> None:
    respx.get(f"{BASE}/df-api/v1/projects/392/versions/948/fact-tables/1").mock(
...
async def test_get_relationship(service: SemanticService) -> None:
    respx.get(f"{BASE}/df-api/v1/projects/392/versions/948/relationships/1").mock(
        return_value=httpx.Response(200, json=RELATIONSHIP_DETAIL)
    )
    result = await service.get_relationship(392, 948, 1)
    assert result["id"] == 1
    assert result["fact_table_id"] == 1
    assert result["dimension_group_id"] == 1
...
async def test_df_api_cache_hit(service: SemanticService) -> None:
    ...
    route = respx.get(f"{BASE}/df-api/v1/projects/392/versions/948/data-marts")
