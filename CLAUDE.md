...
**DF API v2 (`/df-api/v2/`)** — all entities under unified v2 surface:
- `GET /df-api/v2/projects`
- `GET /df-api/v2/projects/{projectId}/versions`
- `GET .../measures` (`include_sql=true` for SQL code generation)
- `GET .../dimensions`
- `GET .../facts`
- `GET .../rmd` (semantic RMD)
...
- `GET .../fact-tables`, `GET .../fact-tables/{id}` (`include_dependencies=true`)
- `GET .../relationships`, `GET .../relationships/{id}` (filters: `fact_table_id`, `dimension_group_id`)
- `GET .../rmd` (consolidated RMD export, `include_sql=true`)

**Rate limiting**: 5 req/60s per API key, HTTP 429 → `DATAFORGE_RATE_LIMIT_EXCEEDED` (no retry)

### MCP Tools

**Semantic tools**: `df_health`, `df_list_projects`, `df_list_versions`, `df_get_measures` (`include_sql`), `df_get_dimensions`, `df_get_facts`, `df_get_rmd`, `df_refresh_cache`

**Data model tools**: `df_list_data_marts`, `df_get_data_mart`, `df_get_data_mart_view`, `df_generate_sql`, `df_list_connections`, `df_get_connection`, `df_get_connection_schema`, `df_list_dimension_groups`, `df_get_dimension_group`, `df_list_fact_tables`, `df_get_fact_table` (`include_dependencies`), `df_list_relationships` (`fact_table_id`, `dimension_group_id` filters), `df_get_relationship`, `df_get_consolidated_rmd` (`include_sql`)
...
- No retry on 400/401/403/404/429
